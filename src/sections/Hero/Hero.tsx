...
const Hero = () => {
    ...
    const videoRefresh = () => {
        if (ref.current) {
            ref.current.src = `${t('hero.src')}?autoplay=1&loop=1&mute=1&modestbranding=1&iv_load_policy=3&controls=1&disablekb=1&fs=0&color=white`
        }
    }
    ...
    return (<section dir={i18next.dir()} className="pt-10 flex flex-col justify-center items-center mx-3 md:mx-10 px-4 border-l-[2rem] border-pop">
        ...
        <div className="relative w-full">
            ...
            <iframe ref={ref} className="w-10/12 mx-auto aspect-video" src={`${t('hero.src')}?autoplay=1&loop=1&mute=1&modestbranding=1&iv_load_policy=3&controls=1&disablekb=1&fs=0&color=white`}></iframe>
        </div>
    </section>)
}
